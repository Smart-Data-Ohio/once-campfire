...
The automated path keeps the same ordering rule as the manual one: the migration is rehearsed against a *copy* of the frozen database, in a throwaway container with no network, **before** the live application is touched. The checks that run after the cutover are read-only, because by then the new image is serving and may have accepted writes. If the release does not complete, the recovery phase restores the previous image only when the live database is provably byte-for-byte unchanged since the freeze; if anything was written it stops the application, refuses to restore the database, and reports what an operator must decide. It leaves the feed timer paused either way.
