...
class Huddle::RoomServiceTest < ActiveSupport::TestCase
  ...
  test "removes only the requested participant using a room-scoped admin token" do
    stub_request(:post, "https://livekit.example.test/twirp/livekit.RoomService/RemoveParticipant")
      .to_return(status: 200, body: "{}")

    Huddle::RoomService.new.remove_participant(room_name: "opaque-room", identity: "opaque-participant")

    assert_requested :post, "https://livekit.example.test/twirp/livekit.RoomService/RemoveParticipant" do |actual|
  ...
  test "deletes only the requested room using a room-create token" do
    stub_request(:post, "https://livekit.example.test/twirp/livekit.RoomService/DeleteRoom")
      .to_return(status: 200, body: "{}")

    Huddle::RoomService.new.delete_room(room_name: "opaque-room")

    assert_requested :post, "https://livekit.example.test/twirp/livekit.RoomService/DeleteRoom" do |actual|
  ...
  test "raises a safe error without following redirects or retaining a response body" do
    request = stub_request(:post, "https://livekit.example.test/twirp/livekit.RoomService/RemoveParticipant")
