...
**Status:** Stage channels shipped with host, speaker, and listener roles, token and grant-revocation enforcement for listeners, hand raising, and host moderation from the stage panel; see [stage channels](docs/stage-channels.md). Streaming shipped with explicit presenter/viewer behavior and quality controls; see [streaming](docs/streaming.md). Agent boards slice 1 shipped: boards for people with posts, status/owner/tags/result, list and board renderings, and inbox updates; see [agent boards](docs/boards.md).
